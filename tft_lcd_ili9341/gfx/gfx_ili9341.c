...
void gfx_fillRect(int x, int y, int w, int h, uint16_t color)
{
    ...
    // Preenche em blocos de linhas usando LCD_WriteBitmap
    #define FILL_BUF_SIZE 240
    static uint16_t line_buf[FILL_BUF_SIZE];

    int buf_w = cw < FILL_BUF_SIZE ? cw : FILL_BUF_SIZE;
    for (int i = 0; i < buf_w; i++)
        line_buf[i] = color;

    for (int row = y0; row < y1; row++)
        LCD_WriteBitmap(x0, row, cw, 1, line_buf);
}
