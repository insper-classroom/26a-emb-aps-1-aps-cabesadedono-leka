...
void initSPI()
{
	spi_init(ili9341_spi, 10000000);
	spi_set_format(ili9341_spi, 16, SPI_CPOL_0, SPI_CPHA_0, SPI_MSB_FIRST);
...
void ILI9341_WriteData(uint8_t *buff, size_t buff_size)
{
	...
	spi_write_blocking(ili9341_spi, ptr, buff_size);
}

void ILI9341_SendCommand(uint8_t commandByte, const uint8_t *dataBytes,
						 uint8_t numDataBytes)
{
	ILI9341_Select();

	ILI9341_WriteCommand(commandByte);
	ILI9341_WriteData((uint8_t *)dataBytes, numDataBytes);
...
void LCD_setAddrWindow(uint16_t x, uint16_t y, uint16_t w, uint16_t h)
{
	...
	ILI9341_WriteData((uint8_t *)&xa, sizeof(xa));  // cast aqui

	ILI9341_WriteCommand(ILI9341_PASET);
	ILI9341_WriteData((uint8_t *)&ya, sizeof(ya));  // cast aqui
